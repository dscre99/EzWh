...
function get_test_descriptor_by_ID(req,res) {
    (empty_body_check(req.body) && req.params.id != undefined) ? get_test_descriptor_by_ID_DB(req.params.id).then((test_descriptor) => {
...
function post_test_descriptor(req, res) {
    empty_body_check(req.body) ? res.status(500).json("Body is empty!") : post_test_descriptor_DB(req.body).then((test_descriptor) => {
        res.status(200).json(test_descriptor);
...
function put_test_descriptor_by_ID(req, res) {
    empty_body_check(req.body) ? res.status(500).json("Body is empty!") : put_test_descriptor_by_ID_DB(req.params.id, req.body).then(() => {
