...
const { get_test_results_DB } = require('./Test_result_DAO');
...
//     (isBodyEmpty(req.body) && req.params.id != undefined) ? get_test_result_with_id_from_rfid_DB(req.params.id).then((test_result) => {
...
//     isBodyEmpty(req.body) ? res.status(500).json("Body is empty!") : put_test_result_with_id_from_rfid_DB(req.params.id, req.body).then(() => {
...
//     delete_test_result_with_id_from_rfid_DB(req.params.id).then(() => {
...
module.exports = {get_test_results}