...
app.get('/api/userinfo', async (req, res) => {
  ...
  let tmpUserDAO = new UserDAO(firstDBtest);
...
app.get('/api/users', async (req, res) => {
  ...
  let getUsersPromise = new UserDAO(firstDBtest).getUsers();
...
app.post('/api/newUser', async (req, res) => {
  ...
  if (Object.keys(tmp_user_data).length != requiredKeys.length) {
    ...
    if(!userTypes.includes(tmp_user_data['type'])){
      return res.status(422).end(); // 422 Unprocessable Entity
    }
  }
...
app.post('/api/managerSessions', async (req, res) => {
  ...
  let managerSessionsPromise = new UserDAO(firstDBtest).userSession(managerData);
...
app.post('/api/customerSessions', async (req, res) => {
  ...
  let customerSessionsPromise = new UserDAO(firstDBtest).userSession(customerData);
...
app.post('/api/supplierSessions', async (req, res) => {
  ...
  let supplierSessionsPromise = new UserDAO(firstDBtest).userSession(supplierData);
...
app.post('/api/clerkSessions', async (req, res) => {
  ...
  let clerkSessionsPromise = new UserDAO(firstDBtest).userSession(clerkData);
...
app.post('/api/qualityEmployeeSessions', async (req, res) => {
  ...
  let qualityEmployeePromise = new UserDAO(firstDBtest).userSession(qualityEmployeeData);
...
app.post('/api/deliveryEmployeeSessions', async (req, res) => {
  ...
  let deliveryEmployeePromise = new UserDAO(firstDBtest).userSession(deliveryEmployeeData);
...
app.put('/api/users/:username', async (req, res) => {
  ...
  if(valid) {
    ...
    if(valid) {
      let putUserPromise = new UserDAO(firstDBtest).modifyUserType(userData);
...
app.delete('/api/users/:username/:type', async (req, res) => {
  ...
  if(!valid) {
    ...
    let deleteUserPromise = new UserDAO(firstDBtest).deleteUser(userData);
